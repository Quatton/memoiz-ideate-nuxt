...
export default defineEventHandler(async (event) => {
  ...
  const completion = res.completions[0].data.text as string;
  let parsed = "";
  try {
    parsed = JSON.parse(completion.trim()).question as string;
  } catch (e) {
    const question = completion.trim().match(/"question": "(.*)"/)?.[1];
    if (question) {
      parsed = question;
    }
  }

  console.log(prompt2(code));
